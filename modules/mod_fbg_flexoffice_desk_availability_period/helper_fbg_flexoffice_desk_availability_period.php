...
class ModFbgFlexofficeDeskAvailabilityPeriodHelper
{
    public static function getAvailabilityPeriods($limit = 20)
    {
        ...
        try {
            ...
            $tableReal = $prefix . 'desk_availability_period';
        ...
        // Cleanup: unset desk_availability_period_id for desks whose period has ended
        // (Some installs rely on desks.desk_availability_period_id so keep data consistent.)
        try {
        ...
        try {
            $db->setQuery('DESCRIBE ' . $db->quoteName($db->getPrefix() . 'desk'));
